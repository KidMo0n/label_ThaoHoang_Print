...
#    8. systemd service: thaohoang-print-label
#
#  Cách dùng:
#    chmod +x install.sh
#    sudo ./install.sh
...
# Cấu hình CUPS cho phép truy cập LAN
...
# Hiện danh sách các thiết bị USB đang thấy
echo "  Thiết bị USB CUPS hiện có:"
lpinfo -v 2>/dev/null | grep -iv "xprinter\|XP-3\|XP-4" | grep -i "usb" | sed 's/^/    /' || echo "    (không có)"
echo ""

read -r -p "  Nhập tên CUPS cho máy in A4 (VD: Canon-LBP2900, HP-LaserJet) hoặc Enter bỏ qua: " A4_NAME
if [ -n "$A4_NAME" ]; then
  ...
  # Cập nhật tên A4 trong server.js
  if [ "$KHU" = "A" ]; then
    sed -i "s/const PRINTER_A4_A = '[^']*'/const PRINTER_A4_A = '$A4_NAME'/" "$APP_DIR/server.js" \
      && info "Đã cập nhật PRINTER_A4_A = '$A4_NAME' trong server.js"
  elif [ "$KHU" = "B" ]; then
    sed -i "s/const PRINTER_A4_B = '[^']*'/const PRINTER_A4_B = '$A4_NAME'/" "$APP_DIR/server.js" \
      && info "Đã cập nhật PRINTER_A4_B = '$A4_NAME' trong server.js"
  fi
else
  warn "Bỏ qua máy in A4 — cập nhật thủ công sau trong server.js"
  info "Sau khi biết tên CUPS: lpstat -a"
  info "Sửa PRINTER_A4_A / PRINTER_A4_B trong $APP_DIR/server.js"
fi
...
# ════════════════════════════════════════════════════════
#  BƯỚC 6: LibreOffice headless (in DOCX/ODT/XLSX)
# ════════════════════════════════════════════════════════
...
echo ""
echo "  Tunnel tự route tất cả path — Express xử lý /in_label và /in_a4."
echo ""
echo "  Cài cloudflared nếu chưa có:"
